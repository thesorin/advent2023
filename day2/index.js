...
const rules = {
  // cube color : number invalidates if cubes of the color are over this number
  red: 12,
  green: 13,
  blue: 14,
};

const parseGame = (gameInput) => {
  const [game, rounds] = gameInput.split(":");
  const [gameId] = game.match(/\d+/g);
  return {
    id: +gameId,
    rounds: rounds.split(";").map((roundRaw) => {
      const round = {
        red: 0,
        green: 0,
        blue: 0,
      };
      roundRaw.split(",").forEach((cube) => {
...
const filterInvalidGames = ({ rounds }) => {
  return !rounds.some((round) => {
    // we look for rounds that invalidate the game
    return Object.keys(rules).some((color) => round[color] > rules[color]);
  });
};
...
const getMinimumCubes = (game) => {
  ...
  rounds.forEach((round) => {
    Object.keys(round).forEach((color) => {
      minimum[color] = Math.max(minimum[color], round[color]);
    });
  });
  const cubes = Object.values(minimum).reduce(
...
const parsedGames = games.map(parseGame).map(getMinimumCubes);

const parsedGamesPuzzleOne = parsedGames.filter(filterInvalidGames);
const gameIds = parsedGamesPuzzleOne.map(({ id }) => id);
const sumOfGameIds = gameIds.reduce((total, id) => total + id, 0);

const sumOfCubes = parsedGames.reduce((t, { cubes }) => t + cubes, 0);

// const cubeaPuzzleTwo = parsedGamesPuzzleOne.reduce();

console.log("Puzzle 1", sumOfGameIds);
console.log("Puzzle 2", sumOfCubes);
