// There is a catch here as the scenario is that lines as 'oneight'
// should be treated as 18 and not '1ight'
...
const wordsAndDigits = Object.entries(digitsMap).reduce(
  (wordsAndDigits, entries) => wordsAndDigits.concat(entries),
  []
);
...
const extractNumbersFromString = (str) => {
  ...
  wordsAndDigits.forEach((wd) => {
    const regex = new RegExp(wd, "g");
    for (match of str.matchAll(regex)) {
      occurrences.push({
        value: wd,
        index: match.index,
      });
  ...
  const toDigits = sorted.map(({ value }) => digitsMap[value] || value);

  return toDigits.join("");
};

const lines = input.split("\n").filter(Boolean);

const getLineCalibrationCoordonates = (line, swapStrinsToDigits = false) => {
  let lineCopy = line;
  if (swapStrinsToDigits) {
    lineCopy = extractNumbersFromString(lineCopy);
  }
...
const calibrateCoordonates = (lines, replaceStringNums = false) =>
  lines
    .map((l) => getLineCalibrationCoordonates(l, replaceStringNums))
    .reduce((a, b) => a + b, 0);

const initialCalibration = calibrateCoordonates(lines);
const newCalibration = calibrateCoordonates(lines, true);

console.log("Puzzle 1 calibration: ", initialCalibration);
console.log("Puzzle 2 calibration: ", newCalibration);
